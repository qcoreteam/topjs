...
describe('loader/StandardAutoloader测试用例', function() {
   ...
   //    });
   //    let filename = loader.transformClassNameToFilename('TopJs.loader.AutoloaderFactory',base);
   //    assert.equal(filename, process.cwd()+"/lib/TopJs/loader/AutoloaderFactory.js");
   // });
   //
   // it("测试normalizeDirectory", function(){
   ...
   it("测试loadClass", function(){
      ...
      Cls = loader.loadClass("TopJs_loader_StandardAutoloader", StandardAutoloader.LOAD_PREFIX);
      assert.equal(StandardAutoloader.ACT_AS_FALLBACK, "fallback_autoloader");
   });
   //
